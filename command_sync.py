...
def get_admin_commands(lang: str = "en") -> list[BotCommand]:
    ...
    if ENABLE_WHITE_LABEL:
        extra = {
            "en": [
                BotCommand("botconnectreq", "🤖 Pending connected-bot requests"),
                BotCommand("connected_bots", "📋 Manage connected bots"),
            ],
            "ru": [
                BotCommand("botconnectreq", "🤖 Заявки на подключение ботов"),
                BotCommand("connected_bots", "📋 Управление подключёнными ботами"),
            ],
            "uz": [
                BotCommand("botconnectreq", "🤖 Bot ulash so‘rovlari"),
                BotCommand("connected_bots", "📋 Ulangan botlarni boshqarish"),
            ],
        }
